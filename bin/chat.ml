let sockaddr_with_secure_port sockaddr secure_port = match sockaddr with
  ...
  | Ok () ->
    ...
    | Ok () ->
      ...
      let rec go lines recv =
        Fiber.npick
          [ begin fun () -> Fiber.wait recv >>| fun v -> `Recv v end
          ; begin fun () -> Fiber.getline Unix.stdin >>| fun v -> `Send v end ]
        >>= function
        | `Send (Some line) ->
          List.iter (Fmt.pr "<~ %s\n%!") lines ;
          let send = Fmt.str "%s\r\n" line in
