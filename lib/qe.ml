...
let create capacity =
  ...
  { rd_cursor = 0; wr_cursor = 0; capacity; buffer = Bstr.create capacity }
...
let grow t want =
  ...
  if capacity' > Bstr.length t.buffer then (
    ...
    if len1 > 0 then (
      Bstr.blit t.buffer ~src_off:rd_cursor buffer' ~dst_off:0 ~len:len0;
      Bstr.blit t.buffer ~src_off:0 buffer' ~dst_off:len0 ~len:len1)
    else Bstr.blit t.buffer ~src_off:rd_cursor buffer' ~dst_off:0 ~len:size';
    t.buffer <- buffer';
    t.wr_cursor <- size';
    t.capacity <- capacity';
    t.rd_cursor <- 0)
...
let compress t =
  ...
  (* TODO(dinosaure): we probably can optimize this part and avoid
     ...
     is not handled so. *)
  (if len1 > 0 then (
     let buffer' = Bstr.create t.capacity in
     Bstr.blit t.buffer ~src_off:rd_cursor buffer' ~dst_off:0 ~len:len0;
     Bstr.blit t.buffer ~src_off:0 buffer' ~dst_off:len0 ~len:len1;
     t.buffer <- buffer')
   else
...
let push t ?(off = 0) ?len bstr =
  ...
  if len1 > 0 then (
    Bstr.blit bstr ~src_off:off t.buffer ~dst_off:wr_cursor ~len:len0;
    Bstr.blit bstr ~src_off:(off + len0) t.buffer ~dst_off:0 ~len:len1)
  else Bstr.blit bstr ~src_off:off t.buffer ~dst_off:wr_cursor ~len;
...
let push_string t str =
  ...
  let len1 = len - len0 in
  if len1 > 0 then (
    Bstr.blit_from_string str ~src_off:0 t.buffer ~dst_off:wr_cursor ~len:len0;
    Bstr.blit_from_string str ~src_off:len0 t.buffer ~dst_off:0 ~len:len1)
  else Bstr.blit_from_string str ~src_off:0 t.buffer ~dst_off:wr_cursor ~len;
  t.wr_cursor <- t.wr_cursor + len
...
let peek t =
  ...
  if len = 0 then []
    ...
    if len1 > 0 then
      [
        Bstr.(copy (sub t.buffer ~off:rd_cursor ~len:len0));
        Bstr.(copy (sub t.buffer ~off:0 ~len:len1));
      ]
    else [ Bstr.(copy (sub t.buffer ~off:rd_cursor ~len)) ]
...
let keep t ?(off = 0) ?len bstr =
  ...
  let len1 = len - len0 in
  if len1 > 0 then (
    Bstr.blit t.buffer ~src_off:rd_cursor bstr ~dst_off:off ~len:len0;
    Bstr.blit t.buffer ~src_off:0 bstr ~dst_off:(off + len0) ~len:len1)
  else Bstr.blit t.buffer ~src_off:rd_cursor bstr ~dst_off:off ~len
...
let keep_bytes t ?(off = 0) ?len buf =
  ...
  let len1 = len - len0 in
  if len1 > 0 then (
    Bstr.blit_to_bytes t.buffer ~src_off:rd_cursor buf ~dst_off:off ~len:len0;
    Bstr.blit_to_bytes t.buffer ~src_off:0 buf ~dst_off:(off + len0) ~len:len1)
  else Bstr.blit_to_bytes t.buffer ~src_off:rd_cursor buf ~dst_off:off ~len
