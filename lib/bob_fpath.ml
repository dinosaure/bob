...
external is_windows : unit -> bool = "bob_is_windows"
...
module Windows = struct
    ...
    | seg1, rest -> (
        ...
        | '?' when seg1_len = 1 -> (
            ...
            | seg2, rest -> (
                ...
                then ensure_root rest
                else if not (String.Sub.equal_bytes seg2 (String.sub "UNC"))
                then
                  (* \\?\server\share\ *)
                  match parse_seg (String.Sub.tail rest) with
                  | seg, _ when String.Sub.is_empty seg -> None
  ...
  let sub_split_volume p =
    ...
    if not (is_unc_path p) then
      ...
      let bslash ~start =
        match String.find ~start (Char.equal '\\') p with
        | None -> assert false
        | Some i -> i
      in
...
let _sub_last_non_empty_seg p =
  (* returns empty on roots though *)
...
let sub_last_non_empty_seg_posix p = _sub_last_non_empty_seg (String.sub p)

let sub_last_non_empty_seg =
  if windows then sub_last_non_empty_seg_windows
  else sub_last_non_empty_seg_posix

let is_rel_seg = function "." | ".." -> true | _ -> false

let sub_is_rel_seg seg =
  match String.Sub.length seg with
  | 1 when String.Sub.get seg 0 = '.' -> true
  | 2 when String.Sub.get seg 0 = '.' && String.Sub.get seg 1 = '.' -> true
  | _ -> false

let sub_is_dir_seg seg =
  match String.Sub.length seg with
  | 0 -> true
  | 1 when String.Sub.get seg 0 = '.' -> true
  | 2 when String.Sub.get seg 0 = '.' && String.Sub.get seg 1 = '.' -> true
...
let validate_and_collapse_seps p =
  ...
  in
  let start =
    (* Allow initial double sep for POSIX and UNC paths *)
...
let append_posix p0 p1 =
  if p1.[0] = dir_sep_char (* absolute *) then p1
  else
    let sep =
...
let append_windows p0 p1 =
  if Windows.is_unc_path p1 || Windows.has_drive p1 then p1
  else if p1.[0] = dir_sep_char then (* absolute *) p1
  else
    let sep =
      if p0.[String.length p0 - 1] = dir_sep_char then "" else dir_sep
    in
...
let filename p =
  match String.Sub.to_string (sub_last_seg p) with
  | "" | "." | ".." -> ""
  | filename -> filename
...
let _basename p =
  match String.Sub.to_string (_sub_last_non_empty_seg p) with
  | "." | ".." -> ""
  | basename -> basename
...
let _parent p =
  ...
  if sub_is_rel_seg seg then [ p; dsep; dotdot_dir_sub ]
  else if String.Sub.is_empty dir then [ dot_dir_sub ]
  else [ dir ]

let parent_windows p =
  let vol, path = Windows.sub_split_volume p in
  if sub_is_root path then p
  else String.Sub.(base_string @@ concat (vol :: _parent path))

let parent_posix p =
  if Posix.is_root p then p
  else String.Sub.(base_string @@ concat (_parent (String.sub p)))
...
let rem_empty_seg_windows p =
  ...
  if sub_is_root path then p
    ...
    if String.get p max <> dir_sep_char then p
    else String.with_index_range p ~last:(max - 1)

let rem_empty_seg_posix p =
  match String.length p with
  | 1 -> p
  | 2 ->
      if p.[0] <> dir_sep_char && p.[1] = dir_sep_char then String.of_char p.[0]
      else p
  | len ->
      let max = len - 1 in
      if p.[max] <> dir_sep_char then p
      else String.with_index_range p ~last:(max - 1)

let rem_empty_seg =
  if windows then rem_empty_seg_windows else rem_empty_seg_posix

let normalize_rel_segs segs =
  (* result is non empty but may be [""] *)
...
let normalize_segs = function
  ...
  | segs -> (
      match normalize_rel_segs segs with [ "" ] -> [ "."; "" ] | segs -> segs)
...
let is_prefix prefix p =
  if not (String.is_prefix prefix p) then false
  else
    ...
    if prefix.[suff_start - 1] = dir_sep_char then true
    else if suff_start = String.length p then (* suffix empty *) true
    else p.[suff_start] = dir_sep_char

let _prefix_last_index p0 p1 =
  (* last char index of segment-based prefix *)
  let l0 = String.length p0 in
  let l1 = String.length p1 in
  let p0, p1, max = if l0 < l1 then (p0, p1, l0 - 1) else (p1, p0, l1 - 1) in
  let rec loop last_dir_sep i p0 p1 =
    match i > max || p0.[i] <> p1.[i] with
    | false ->
    ...
    | true -> (
        if i = 0 then None
          ...
          let last = i - 1 in
          if last_dir_sep = last then Some last
          else
            match last = max with
            | true ->
                if l1 = l0 then Some last
                else if p1.[i] = dir_sep_char then Some last
                else if last_dir_sep <> -1 then Some last_dir_sep
                else None
            | false -> if last_dir_sep <> -1 then Some last_dir_sep else None)
  in
  loop (-1) 0 p0 p1

let find_prefix_windows p0 p1 =
  match _prefix_last_index p0 p1 with
  | None -> None
  | Some i ->
...
let rem_prefix prefix p =
  match is_prefix prefix p with
  | false -> None
  | true -> (
...
let _relativize ~root p =
  let root =
    (* root is always interpreted as a directory *)
    let root = normalize root in
    if root.[String.length root - 1] = dir_sep_char then root
    else root ^ dir_sep
  in
  ...
  let rec walk root p =
    ...
    | root, p ->
        ...
        let segs =
          List.fold_left (fun acc _ -> dotdot :: acc) p (List.tl root)
        in
...
let relativize_windows ~root p =
  ...
  if not (String.Sub.equal_bytes rvol pvol) then None
  else
...
let is_rooted ~root p =
  match relativize ~root p with
  | None -> false
...
let is_rel_windows p =
  if Windows.is_unc_path p then false
  else p.[Windows.non_unc_path_start p] <> dir_sep_char
...
let is_current_dir_posix ?(prefix = false) p =
  match prefix with
...
let is_parent_dir_posix ?(prefix = false) p =
  match prefix with
...
let has_ext e p =
  ...
  else if not String.(Sub.is_suffix (sub e) ext) then false
...
let exists_ext ?(multi = false) p =
  ...
  if multi then String.Sub.exists eq_ext_sep (String.Sub.tail ext)
  else not (String.Sub.is_empty ext)

let add_ext e p =
  if String.is_empty e then p
  else if not (is_seg e) then invalid_arg (err_invalid_ext e)
  else
    let seg = sub_last_non_empty_seg p in
    if sub_is_dir_seg seg then p
    else
...
let _split_ext ?multi p =
  let ext = sub_get_ext ?multi p in
  if String.Sub.is_empty ext then (p, ext)
  else
    let before_ext = String.Sub.start_pos ext - 1 in
    if String.Sub.stop_pos ext = String.length p then
      (String.with_index_range p ~last:before_ext, ext)
    else
