...
CAMLprim value bob_retrieve_error(value unit) {
  CAMLparam1(unit);
  CAMLlocal1(res);
...
CAMLprim value bob_bigstring_read(value fd, value ba, value off, value len) {
  int ret;
...
CAMLprim value bob_bigstring_write(value fd, value ba, value off, value len) {
  int ret;
  uint8_t *buf = ((uint8_t *)Caml_ba_data_val(ba)) + Long_val(off);
  long max = Long_val(len);
...
CAMLprim value bob_is_windows(__unit()) {
 ...
 * FreeBSD as Unix. */
CAMLprim value bob_is_freebsd(__unit()) {
#if defined(__ESPERANTO__)
#include "cosmopolitan.h"
 ...
 * dynamically. */
CAMLprim value bob_set_nonblock(value fd, value mode) {
#if defined(__ESPERANTO__)
#include "cosmopolitan.h"
  if (IsWindows()) {
